...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Slovene Task A model.")
    parser.add_argument("--train", default="datasets/slovene/train/SemEval2018-T3-train-taskA_emoji.txt")
    parser.add_argument("--test", default="datasets/slovene/test_TaskA/SemEval2018-T3_input_test_taskA_emoji.txt")
    parser.add_argument("--gold", default="datasets/slovene/goldtest_TaskA/SemEval2018-T3_gold_test_taskA_emoji.txt")
    ...
    parser.add_argument("--model-out", default="slovene_pipeline/model_taskA.joblib")
    parser.add_argument("--pred-out", default="slovene_pipeline/predictions_taskA.txt")
    ...
    if args.use_api_features:
        api_train = build_api_features(train_corpus, task="A")
    ...
    cv_f1 = f1_score(y_train, y_cv, pos_label=1)
    print(f"Task A CV F1 (pos_label=1): {cv_f1:.4f}")
    ...
    if args.use_api_features:
        api_test = build_api_features(test_corpus, task="A")
    ...
    if len(gold_corpus) == len(gold_labels):
        ...
        if args.use_api_features:
            api_gold = build_api_features(gold_corpus, task="A")
            if api_gold.shape[1] > 0:
                x_gold = combine_features(x_gold, api_gold)
        y_gold_pred = clf.predict(x_gold)
        gold_f1 = f1_score(np.array(gold_labels), y_gold_pred, pos_label=1)
        print(f"Task A Gold F1 (pos_label=1): {gold_f1:.4f}")
